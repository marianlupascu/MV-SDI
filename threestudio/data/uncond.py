...
class RandomCameraDataModuleConfig:
    ...
    antithetic_pairs: bool = False  # legacy alias, mapped to antithetic_mode="azimuth"
    antithetic_mode: str = "none"  # "none" | "azimuth" | "mixed" | "octahedral" | "random_axis"
...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        # determine effective antithetic mode (with legacy alias)
        anti_mode = self.cfg.antithetic_mode
        if anti_mode == "none" and self.cfg.antithetic_pairs:
            anti_mode = "azimuth"

        def _wrap_azim(a: torch.Tensor) -> torch.Tensor:
            a = torch.where(a > 180.0, a - 360.0, a)
            a = torch.where(a < -180.0, a + 360.0, a)
            return a

        # sample azimuth angles from a uniform distribution bounded by azimuth_range
        azimuth_deg: Float[Tensor, "B"]
        if anti_mode == "azimuth" and self.batch_size >= 2:
            ...
            opposite_azimuth = _wrap_azim(base_azimuth + 180.0)
            azimuth_deg = torch.cat([base_azimuth, opposite_azimuth], dim=0)
            if self.batch_size % 2 == 1:
                ...
                azimuth_deg = torch.cat([azimuth_deg, extra], dim=0)
            elevation_deg = elevation_deg[:half].repeat(2)
            if self.batch_size % 2 == 1:
                elevation_deg = torch.cat(
                    [elevation_deg, elevation_deg[:1]], dim=0
                )
            elevation = elevation_deg * math.pi / 180
        elif anti_mode == "mixed" and self.batch_size == 4:
            # Pair 1 (azimuth-flip): same elevation eps, azim phi vs phi+180
            # Pair 2 (elevation-flip): same azimuth phi', elev_max vs elev_min
            phi_a = (
                torch.rand(1)
                * (self.azimuth_range[1] - self.azimuth_range[0])
                + self.azimuth_range[0]
            )
            phi_a_opp = _wrap_azim(phi_a + 180.0)
            eps_mid = (
                torch.rand(1)
                * (self.elevation_range[1] - self.elevation_range[0])
                + self.elevation_range[0]
            ...
            eps_max = torch.tensor([float(self.elevation_range[1])])
            eps_min = torch.tensor([float(self.elevation_range[0])])
            azimuth_deg = torch.cat([phi_a, phi_a_opp, phi_b, phi_b], dim=0)
            elevation_deg = torch.cat([eps_mid, eps_mid, eps_max, eps_min], dim=0)
            elevation = elevation_deg * math.pi / 180
        elif anti_mode == "octahedral" and self.batch_size == 6:
            # 3 axis-aligned pairs relative to base azimuth phi:
            #   +X: (phi, 0),  -X: (phi+180, 0)
            #   +Y: (phi+90, 0), -Y: (phi+270, 0)
            #   +Z: (random_azim, eps_max), -Z: (random_azim, eps_min)
            phi = (
                torch.rand(1)
                * (self.azimuth_range[1] - self.azimuth_range[0])
                + self.azimuth_range[0]
            ...
            phi_x_pos = phi
            phi_x_neg = _wrap_azim(phi + 180.0)
            phi_y_pos = _wrap_azim(phi + 90.0)
            phi_y_neg = _wrap_azim(phi + 270.0)
            phi_z_pos = (
                torch.rand(1)
                * (self.azimuth_range[1] - self.azimuth_range[0])
                + self.azimuth_range[0]
            ...
            azimuth_deg = torch.cat(
                [phi_x_pos, phi_x_neg, phi_y_pos, phi_y_neg, phi_z_pos, phi_z_neg],
                dim=0,
            )
            ...
            eps_min = torch.tensor([float(self.elevation_range[0])])
            elevation_deg = torch.cat(
                [zero, zero, zero, zero, eps_max, eps_min], dim=0
            )
            elevation = elevation_deg * math.pi / 180
        elif anti_mode == "random_axis" and self.batch_size >= 2:
            # Phase 4.3 -- random-rotated antithetic pairs. We sample a
            # random great-circle direction on the unit sphere and place
            # the pair at antipodal points along it (reflection through the
            # origin: $(\phi, \theta) \to (\phi + 180^\circ, -\theta)$),
            # breaking the implicit object-aligned cardinal-axis assumption
            # of ``mode == "azimuth"``.
            #
            # To keep BOTH poles inside the configured ``elevation_range``
            # (so the 2D prior stays in-distribution for both views), we
            # restrict the sampled axis elevation to the intersection
            # $[\theta_{\min}, \theta_{\max}] \cap [-\theta_{\max}, -\theta_{\min}]$
            # which simplifies to $[\max(-\theta_{\max}, \theta_{\min}),\,
            # \min(\theta_{\max}, -\theta_{\min})]$.
            half = self.batch_size // 2
            axis_azimuth = (
                torch.rand(half)
                * (self.azimuth_range[1] - self.azimuth_range[0])
            ...
            el_lo = max(-float(self.elevation_range[1]), float(self.elevation_range[0]))
            el_hi = min(float(self.elevation_range[1]), -float(self.elevation_range[0]))
            if el_hi <= el_lo:
                # Highly asymmetric elevation_range (e.g. [-10, 45]) ⇒ no
                # intersection. Fall back to a symmetric subinterval around 0.
                rad = min(float(self.elevation_range[1]), -float(self.elevation_range[0]))
                rad = max(rad, 0.0)
                el_lo, el_hi = -rad, rad
            axis_elevation = torch.rand(half) * (el_hi - el_lo) + el_lo
            opp_azimuth = _wrap_azim(axis_azimuth + 180.0)
            opp_elevation = -axis_elevation
            azimuth_deg = torch.cat([axis_azimuth, opp_azimuth], dim=0)
            elevation_deg = torch.cat([axis_elevation, opp_elevation], dim=0)
            elevation = elevation_deg * math.pi / 180
            if self.batch_size % 2 == 1:
                extra_az = (
                    torch.rand(1)
                ...
                azimuth_deg = torch.cat([azimuth_deg, extra_az], dim=0)
                elevation_deg = torch.cat([elevation_deg, extra_el], dim=0)
                elevation = elevation_deg * math.pi / 180
