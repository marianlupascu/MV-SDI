...
class SaverMixin:
    ...
    def save_img_sequence(
        ...
        except (TypeError, ValueError, OSError, ImportError) as e:
            # Older/newer imageio + missing ffmpeg/pyav causes the mp4 writer
            # to silently fall back to TiffWriter (which doesn't accept fps)
            # or raise an unrelated error. Individual PNG frames are already
            # on disk under {img_dir}, which is what the eval pipeline reads,
            # so we degrade the video export to a warning instead of crashing
            # the entire run.
            try:
                import threestudio as _ts
                _ts.warn(
                    f"[save_img_sequence] failed to write {save_path}: {e}. "
                    f"PNG frames are intact at {img_dir}; eval pipeline is unaffected."
                )
            except Exception:
                print(f"[save_img_sequence WARN] {save_path}: {e}  "
                      f"(PNGs intact at {img_dir}; eval unaffected)")
        if name and self._wandb_logger:
            wandb.log(
                {
